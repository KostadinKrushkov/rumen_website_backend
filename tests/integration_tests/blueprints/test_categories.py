...
class TestCategoryAPI:
    ...
    def _assert_category_is_saved(self, expected_category, categories_list=None):
        categories_list = categories_list if categories_list is not None else list(self.gateway.get_all())
        category = categories_list[0] if isinstance(categories_list[0], CategoryDTO) else CategoryDTO.from_dict(
            categories_list[0])

        self._assert_category_matches_expected(expected_category, category)
